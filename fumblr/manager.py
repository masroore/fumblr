...
google_blueprint = make_google_blueprint(
    client_id=GOOGLE_ID, client_secret=GOOGLE_SECRET, scope=["profile", "email"]
)
...
def google_error(blueprint, message, response):
    msg = ("OAuth error from {name}! " "message={msg} response={resp} ").format(
        name=blueprint.name, msg=message, resp=response
    )
    flash(msg, category="error")
