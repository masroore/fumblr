...
def set_username():
    ...
    if not new_username:
        return render_template(
            "settings.html", error={"username": "Please enter a username"}
        )
...
def set_password():
    ...
    if not orig_password or not new_password:
        return render_template(
            "settings.html",
            error={"password": "Please enter your password and new password"},
        )
    ...
    if not current_user.verify_password(orig_password):
        return render_template(
            "settings.html", error={"password": "Incorrect password"}
        )
...
@app.route("/fumblr/uploads/<path:path>")
...
def message(username=None):
    ...
    if request.method == "GET":
        ...
        user_messages = {
            u: [
                ...
                if m["from"] == u
                and m["to"] == current_user.username
                or m["to"] == u
                and m["from"] == current_user.username
            ]
            for u in users
    ...
    if username:
        ...
        return jsonify(
            message=render_template("component/message.html", message=msg.get_data()),
            user=username,
        )
...
def new_post():
    ...
    image_ids = [
        int(request.form.get(item)) for item in request.form if "image" in item
    ]
    if image_ids:
...
@login_required
def dashboard(post_count: int = 0):
    """
...
def user(username, post_count=0):
    ...
    posts_query = (
        user.posts.order_by(Post.created.desc()).offset(post_count).limit(POST_LIMIT)
    )
    posts_data = Post.get_posts_data(posts_query)
...
def search():
    ...
    return render_template(
        "search.html", posts=posts_data, users=users_data, tags=tags_data, search=q
    )
